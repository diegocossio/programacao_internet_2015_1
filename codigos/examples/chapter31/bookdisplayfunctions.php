...
// Display the large cover image for a book (or probably the equivalent image for other products)
function showImage($ASIN, $mode)
{
  $ars = getARS('asin', array('asin'=>$ASIN, 'mode'=>$mode));
  $url = $ars->getImageURLLarge($ASIN, $mode);
  $imageFile = getImage($url, 200000); // allow for larger image sizes here
  
  echo '<hr>';
  echo "<p align = 'center'><a href = 'index.php?action=detail&ASIN=$ASIN&mode=$mode'><img src='images/details.gif' alt='View Book Details' border=0></a>";
  echo "<p align = 'center'><a href = 'index.php?action=detail&ASIN=$ASIN&mode=$mode'>
  <img src='$imageFile' alt='View Book Details' border=0></a>";
  echo "<p align = 'center'><a href = 'index.php?action=detail&ASIN=$ASIN&mode=$mode'><img src='images/details.gif' alt='View Book Details' border=0></a>";
  echo '<hr>';
}
...
function showSearch($search, $page, $mode)
{
  ...
  if($products)
    showSummary($products, $page, $ars->_totalResults, $mode, 0, false, $search);
  else
  ...
  else
  {
    ...
    foreach ($products as $product)
    {
      if(!$product || !$product->valid())
        continue;  // there is some odd stuff in the catalogue.  Skip products that are missing vital data.
      $ASIN = $product->ASIN();      
         
      echo '<tr><td colspan = 2><hr></td></tr>';
      $imageFile =getImage($product->imageURLMedium());
     
      echo "<tr><td align = center><a href='index.php?action=detail&ASIN=$ASIN'>
                      <img src= '$imageFile' alt = 'View book details' border = 0>
                      </a></td><td>";
      
      echo '<h3>'.$product->productName().'</h3>';
      echo displayAuthors($product->authors()).'<br>';
      if($product->releaseDate())
        echo $product->releaseDate().'<br>';
      echo '<span class="bookprice">$'.$product->ourPrice().'</span> (list price <span class=listprice>$'.$product->listPrice().'</span>)<br>';

      if($cart) // display quantity and total price if showing shopping cart
      {
      ...
      if($cart)
        echo "<a href = 'index.php?action=deletefromcart&ASIN=$ASIN'><img src = 'images/delete.gif' alt = 'Remove Item From Cart' border = 0></a>";
      else
        echo "<a href = 'index.php?action=addtocart&ASIN=$ASIN'><img src = 'images/addtocart.gif' alt = 'Add to Cart' border = 0></a>";
            
      
      echo ' <a href="index.php?action=detail&ASIN='.$ASIN.'"><img src = "images/details.gif" alt = "View Details" border = 0></a>';
      echo "</td></tr>";    
    }

    echo '</table>';
    
    if($totalProducts>10&&!$cart) // we could break cart contents into pages, but choose not to
    {
      echo 'Go To Page: ';
      ...
      for($i = 1; $i<=$pages; $i++)
      {
        ...
        {
          if($browseNode)
            echo "<a href = 'index.php?action=browsenode&browseNode=$browseNode&page=$i'>$i</a> ";
          if($search)
            echo "<a href = 'index.php?action=search&search=$search&page=$i'>$i</a> ";
        }
      }
    }
  }
...
function showDetail($ASIN, $mode)
{
  ...
  // remember that this section will often be making 5 extra calls to Amazon
